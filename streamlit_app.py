...
from uae_stocks_analysis import (
    ...
    weekly_prep_table, market_indices_summary,
)
...
tab_analysis, tab_weekly = st.tabs(["🔍 تحليل تفصيلي", "📅 التحضير الأسبوعي"])
...
with tab_weekly:
    st.subheader("📅 التحضير الأسبوعي لكل أسهمك")
    st.caption("يجمع الإغلاق الأسبوعي، الإشارات قصيرة المدى، أقرب دعم/مقاومة، وحالة السوق العام - بضغطة وحدة.")

    weekly_button = st.button("🔄 جهّز التحضير الأسبوعي", use_container_width=True)

    if weekly_button:
        if not tickers:
            st.warning("ضيف رمز سهم واحد على الأقل من الإعدادات الجانبية.")
        else:
            with st.spinner("جاري جلب أداء المؤشرات العامة..."):
                idx = market_indices_summary()

            st.markdown("### حالة السوق العام")
            i1, i2 = st.columns(2)
            for col, (name, data) in zip([i1, i2], idx.items()):
                if data:
                    col.metric(name, f"{data['value']:.2f}", f"{data['weekly_change_pct']:+.2f}% أسبوعياً")
                else:
                    col.metric(name, "غير متاح")

            st.markdown("### جدول أسهمك")
            with st.spinner("جاري تجهيز الجدول (قد يأخذ دقيقة حسب عدد الأسهم)..."):
                table = weekly_prep_table(tickers, days, fetch_dividends=fetch_fund)
            st.dataframe(table, use_container_width=True)

            st.markdown("""
**تذكير بخطوات التحضير الكاملة (مو بس الجدول):**
1. راجع جدول الأرباح والتوزيعات لأسهمك هالأسبوع
2. راجع أي أخبار جوهرية عن الشركات
3. قارن أداء القطاعات (عقار، بنوك، طاقة...)
4. اكتب سيناريو دخول/خروج لكل مركز **قبل** بداية التداول
5. راجع نسبة كل سهم من إجمالي محفظتك (تجنب التركز)
            """)
    else:
        st.write("اضغط **جهّز التحضير الأسبوعي** فوق للبدء (يستخدم نفس رموز الأسهم من الإعدادات الجانبية).")
